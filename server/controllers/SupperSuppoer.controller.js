import { SupperSupport } from "../models/SupperSupport.Modal.js";
...
export const createComplaint = async (req, res) => {
  try {
    ...
    const newComplaint = new SupperSupport({
      title,
      complaint,
      sellerId,
...
export const getAllComplaints = async (req, res) => {
  try {
    const complaints = await SupperSupport.find().sort({createdAt: -1})
...
export const bcsupportComplatsforseller = async (req, res) => {
    const {id} = req.params;
      try {
        ...
        const ordecomplants = await SupperSupport.find({ sellerId: id }).sort({ createdAt: -1 });
        if (!ordecomplants) {
...
export const updateComplaintStatus = async (req, res) => {
  try {
    ...
    const complaint = await SupperSupport.findById(id);
