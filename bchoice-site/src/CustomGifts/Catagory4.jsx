...
import { fetchCatagory4 } from "../store/catagores/catagory4";
...
const Catagory4 = () => {
  ...
  const products = useSelector((state) => state.catagory4.items);
  const status = useSelector((state) => state.catagory4.status);
  const error = useSelector((state) => state.catagory4.error);
  useEffect(() => {
    if (status === "idle" && products.length === 0) {
      dispatch(fetchCatagory4({ category: "Tech" }));
    }
      ...
      <div className="container mx-auto md:p-4 max-w-[90rem]">
        <div className="flex flex-col lg:flex-row md:justify-between ">
          <div className="max-w-xl xl:max-w-4xl p-4 md:p-0">
            <h1 className="text-xl md:text-4xl  font-bold  mb-1 mt-6">Latest Tech & Gadgets</h1>
            <p className="text-sm md:text-lg text-gray-600 mb-4 md:mb-6 ">
              Discover the latest tech and cool gadgets made to improve your daily life and keep you up to date.
            </p>
          </div>
        </div>
        <div>
          {/* Filter ma, All, Men, Women, Watches, Earburds, Others */}
          <div className="flex pl-1 md:space-x-4 pb-2 space-x-2  overflow-x-scroll md:overflow-x-hidden scrollbar-hide shrink-0 flex-nowrap">
            <button
            ...
            <button
              onClick={() => setFilterType("Watches")}
              className="md:min-w-[120px] min-w-[90px] px-2 py-1 md:text-sm text-xs bg-gradient-to-r from-orange-400 to-red-400 text-white rounded-sm">
              Watches
            </button>
            <button
              onClick={() => setFilterType("Earburds")}
              className="md:min-w-[120px] min-w-[90px] px-2 py-1 md:text-sm text-xs bg-gradient-to-r from-orange-400 to-red-400 text-white rounded-sm">
              Earburds
            </button>
            <button
        ...
        {/* <div className="fixed bottom-0 left-0 right-0 bg-white border-t py-3 shadow-lg flex justify-between px-8">
          <Link to={"/led-lamps"} className="px-3 py-1 bg-red-500 text-white rounded-sm">
            Previous
          </Link>

          <Link to="/cart" className="px-3 py-1 bg-red-500 text-white rounded-sm">
            CheckOut
          </Link>
        </div> */}
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t py-3 shadow-lg flex justify-center items-center gap-4 md:gap-6 px-8">
          ...
          {/* Skip Button */}
          <Link to="/cart" className="md:px-6 md:py-3 px-3 py-2 text-black border-2 rounded-sm transition-all duration-300 ">
            View Cart
          </Link>
