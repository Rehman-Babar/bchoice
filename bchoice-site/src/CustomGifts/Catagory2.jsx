...
import { fetchCatagory2 } from "../store/catagores/catagory2";
...
const Catagory2 = () => {
  ...
  const products = useSelector((state) => state.catagory2.items);
  const status = useSelector((state) => state.catagory2.status);
  useEffect(() => {
    if (status === "idle" && products.length === 0) {
      dispatch(fetchCatagory2({ category: "Jewelry" }));
    }
      ...
      <div className="container mx-auto md:p-4 max-w-[90rem]">
        <div className="flex flex-col lg:flex-row md:justify-between">
          <div className="max-w-xl xl:max-w-4xl p-4 md:p-0">
            <h1 className="text-xl md:text-4xl  font-bold  mb-1 mt-6">Select Elegant Jewelry:</h1>
            <p className="text-sm md:text-lg text-gray-600 mb-4 md:mb-6 ">
              Here, you can select elegant jewelry with a product quality guarantee.
            </p>
          </div>
        </div>
        <div>
          <div className="flex md:space-x-4 pl-1 space-x-2 pb-2 overflow-x-scroll md:overflow-x-hidden scrollbar-hide shrink-0 flex-nowrap">
            <button
              ...
              className="min-w-[120px] px-2 py-1 md:text-sm text-xs bg-gray-500 text-white rounded-sm">
              All
            </button>
            <button
              onClick={() => setFilterType("Men")}
              className="min-w-[120px] px-2 py-1 md:text-sm text-xs bg-gradient-to-r from-orange-400 to-red-400 text-white rounded-sm">
              Men
            </button>
            <button
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t py-3 shadow-lg flex justify-center items-center md:gap-6 gap-4 px-8">
          ...
          <Link
            ...
            className="md:px-6 md:py-3 px-3 py-2 text-black border-2 rounded-sm transition-all duration-300 ">
            Skip
          </Link>
