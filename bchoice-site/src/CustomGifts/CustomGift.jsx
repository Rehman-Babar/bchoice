...
const CustomGift = () => {
  ...
  const products = useSelector((state) => state.giftProduct.items);
  const status = useSelector((state) => state.giftProduct.status);
  // const error = useSelector((state) => state.giftProduct.error);
  useEffect(() => {
    if (status === "idle" && products.length === 0) {
      dispatch(fetchGiftProduct({ category: "Custom Gift" }));
    }
  ...
  return (
    <>
      <Navbar />
      <Navbar2 />
      <div className="container mx-auto md:p-4 max-w-[90rem] z-10">
        <div className="flex flex-col lg:flex-row md:justify-between">
          <div className="max-w-xl xl:max-w-2xl p-4 md:p-0">
            <h1 className="text-xl md:text-4xl  font-bold  mb-1 mt-6">Select Your Custom Product:</h1>
            <p className="text-sm md:text-lg text-gray-600 mb-4 md:mb-6 ">
              Here, you can select and customize products based on your preferences.
            </p>
          </div>
        </div>
        <div>
          {/* Filter ma, All, Hand Made, Photo Frames, Printing, Resin Art, Others */}
          <div className="flex md:space-x-4 space-x-2 pb-2 pl-1 overflow-x-scroll md:overflow-x-hidden scrollbar-hide shrink-0 flex-nowrap">
            <button
            ...
            <button
              onClick={() => setFilterType("Hand Made")}
            ...
            <button
              onClick={() => setFilterType("Photo Frames")}
              className="md:min-w-[120px] min-w-[90px] px-2 py-1 md:text-sm text-xs bg-gradient-to-r from-orange-400 to-red-400 text-white rounded-sm">
              Photo Frames
            </button>
            <button
              onClick={() => setFilterType("Printing")}
              className="md:min-w-[120px] min-w-[90px] px-2 py-1 md:text-sm text-xs bg-gradient-to-r from-orange-400 to-red-400 text-white rounded-sm">
              Printing
            </button>
            <button
              onClick={() => setFilterType("Resin Art")}
              className="md:min-w-[120px] min-w-[90px] px-2 py-1 md:text-sm text-xs bg-gradient-to-r from-orange-400 to-red-400 text-white rounded-sm">
              Resin Art
            </button>
            <button
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {filteredProducts?.map((product) => (
                <div
                  ...
                  className={`min-w-[150px] cursor-pointer sm:min-w-[180px] bg-white border rounded-lg shadow-md transform transition-all duration-300`}>
                    ...
                    {product.videoUrl.length > 0 && (
                        <span
                          ...
                          onClick={(e) => {
                            ...
                            openVidoeModal(product.videoUrl);
                          }}
                        >
                          <FaRegCirclePlay />
                        </span>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t py-3 shadow-lg flex justify-center items-center gap-4 md:gap-6 px-8">
          {/* Previous Button */}

          {/* Skip Button */}
          <Link to="/gifts" className="md:px-6 md:py-3 px-3 py-2 text-black border-2 rounded-sm transition-all duration-300 ">
            Skip
          </Link>
