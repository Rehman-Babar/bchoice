...
const SupperSupport = () => {
  ...
  const [createModalOpen, setCreateModalOpen] = useState(false); // Modal for creating complaints
  ...
  //     complaint:
  //       "This is a sample complaint about a delivered product issue.",
  //     status: "Solved",
  //     date: "Nov 27, 2024",
  //   },
  //   {
  //     title: "Shipping Issue",
  //     complaint: "The package was dispatched late.",
  //     status: "Pending",
  //     date: "Nov 26, 2024",
  //   },
  // ];
  ...
  const fetchComplants = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/v5/complaints/seller/${user?._id}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BASE_URL}/api/v5/complaints/create`, {
        method: "POST",
        headers: {
...
<div className="bg-white rounded-sm p-3">
  ...
  <div className="flex justify-between items-center mb-4">
    <h1 className="text-lg font-semibold">Support Support</h1>
  </div>
    ...
    {complaints.map((complaint, index) => (
      <tr key={index} className="border-b">
        ...
        <td className="px-4 py-2 border-r">
          <span
            className={`px-2 py-1 lg:text-sm text-[5px] rounded-full ${getStatusClass(
              complaint.status
            )}`}
          >
            {complaint.status}
        ...
        <td className=" text-center">
          <button
            className="text-gray-800 hover:bg-blue-100 lg:text-base text-[5px] p-2 rounded-full"
  ...
  <button
    ...
    className="fixed bottom-4 right-4 bg-[#A95FB8] text-white rounded-full w-12 h-12 flex items-center justify-center shadow-lg hover:[#A99FB1]"
  >
    <svg
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="text-red-500 hover:border-red-700 p-2 rounded-sm"
                >
                  Cancel
                </button>
              </div>
