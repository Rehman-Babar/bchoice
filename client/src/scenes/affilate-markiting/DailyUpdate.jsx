...
const DailyUpdate = () => {
  ...
  const fetchUpdates = async () => {
    try {
      const response = await fetch(`https://admin-server-98to.onrender.com/api/v6/dailyupdate/get/all/ceo`);
      if (response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("https://admin-server-98to.onrender.com/api/v6/dailyupdate/create", {
        method: "POST",
        headers: {
