...
import { fetchOrders, selectNewArrivalOrders } from "state/ordersData/GetOrderHook";
import { formatPostDates } from "utils/date.js";
import OrderModal from "./OrderModal";

function GetOrderForNewArrival() {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(fetchOrders()); // Fetch the orders when component mounts
  }, [dispatch]);

  // Access categorized orders
  const newArrivalOrders = useSelector(selectNewArrivalOrders);
  ...
  return (
    <>
      <div className="p-6">
        <div className="text-center text-black text-xl md:text-3xl my-4">New Arrivals OrdersData</div>
        <div className="overflow-x-auto">
          <table className="min-w-full bg-[#e0affc2a] border border-gray-300">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {newArrivalOrders.map((order, index) => (
                <tr key={index}>
                  ...
                  {/* <td className="px-6 py-4 whitespace-nowrap">{order.shipping_method}</td> */}
...
export default GetOrderForNewArrival;
