...
const PandingWidraw = () => {
  ...
  const fetchPayments = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/v7/payment/unapproved`);
      if (!response.ok) {
 ...
 // Set initial status // Set checkbox status based on payment status
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/v7/payment/unapproved/approve/admin/${selectedPayment._id}`, {
        method: "POST",
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:8000/api/v7/payment/unapproved/admin/${id}`, {
        method: "DELETE",
      });
