  ...
  const fetchComplaints = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/v5/complaints/get/all/ceo`
      );
  ...
  const updateComplaint = async () => {
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/api/v5/complaints/${selectedComplaint._id}/status`,
        { solution, status }
      );
      if (response.status === 200) {
        setComplaints((prev) =>
    ...
    <div className="lg:p-4 p-0 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-sm lg:p-3 p-1">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-lg font-semibold">Supper Support </h1>
        </div>
